from fastapi import APIRouter, HTTPException, Depends, Request, Response
from sqlalchemy.orm import Session
import sys; sys.path.insert(0, "..")
import database, crud
from models import IssueCreate, IssueUpdate, IssueResponse

router = APIRouter(tags=["issues"])


@router.get("/repos/{owner}/{repo}/issues", response_model=list[IssueResponse])
def list_issues(owner: str, repo: str, state: str = "open",
                db: Session = Depends(database.get_db)):
    if not crud.get_repo(db, owner, repo):
        raise HTTPException(status_code=404, detail=f"Repository '{owner}/{repo}' not found")
    return crud.get_repo_issues(db, owner, repo, state=state)


@router.post("/repos/{owner}/{repo}/issues", response_model=IssueResponse,
             status_code=201, name="create_issue")
def create_issue(owner: str, repo: str, issue_in: IssueCreate,
                 request: Request, response: Response,
                 db: Session = Depends(database.get_db)):
    if not crud.get_repo(db, owner, repo):
        raise HTTPException(status_code=404, detail=f"Repository '{owner}/{repo}' not found")
    issue = crud.create_issue(db, owner=owner, repo=repo, author="anonymous",
                              title=issue_in.title, body=issue_in.body, labels=issue_in.labels)
    response.headers["Location"] = str(
        request.url_for("get_issue", owner=owner, repo=repo, number=issue.number)
    )
    return issue


@router.get("/repos/{owner}/{repo}/issues/{number}", response_model=IssueResponse,
            name="get_issue")
def get_issue(owner: str, repo: str, number: int, db: Session = Depends(database.get_db)):
    issue = crud.get_issue(db, owner, repo, number)
    if not issue:
        raise HTTPException(status_code=404, detail=f"Issue #{number} not found")
    return issue


@router.patch("/repos/{owner}/{repo}/issues/{number}", response_model=IssueResponse)
def update_issue(owner: str, repo: str, number: int, issue_in: IssueUpdate,
                 db: Session = Depends(database.get_db)):
    if not crud.get_repo(db, owner, repo):
        raise HTTPException(status_code=404, detail=f"Repository '{owner}/{repo}' not found")
    updates = issue_in.model_dump(exclude_unset=True)
    issue = crud.update_issue(db, owner, repo, number, updates)
    if not issue:
        raise HTTPException(status_code=404, detail=f"Issue #{number} not found")
    return issue
